...
const MapAutoComplete = ({ setState, setShippingVariants }) => {
  const { theme } = useTheme();

  const { order, setOrder, deliveryType } = useShippingContext();

  const { isGoogleMapsLoaded } = useGoogleMaps();
  // store info
  const { storeInfo, isMobile } = useAppContext();
  ...
  const handlePlaceChanged = () => {
    ...
    if (deliveryType === "ship") {
      ...
      if (setShippingVariants) {
        shippings = storeInfo?.shipping?.filter(
