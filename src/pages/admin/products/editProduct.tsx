...
const EditProduct: React.FC<PropsType> = ({
  product,
  close,
  rerenderProducts,
}) => {
  ...
  const [options, setOptions] = useState(
    product?.inStock || [{ qnt: "", size: "" }]
  );
  // varinats
  const [variants, setVariants] = useState([]);
  const [activeVariants, setActiveVariants] = useState(product?.options || []);
  ...
  useEffect(() => {
    ...
    setSex(product?.sex || "Men");
    setBrand(product?.brand || "");
    setColor(product?.color || "Black");
    ...
    setOptions(product?.inStock || [{ qnt: "", size: "" }]);
    setActiveVariants(product?.options || []);
  }, [product?._id]);
  ...
  async function FileUpload(status: any) {
    if (location.search.includes("overview")) {
      return setAlert({
        active: true,
        text: "Edit Product is impossible, because you are in overview mode",
        type: "warning",
      });
    }
    if (
    ...
    const addFileInCloud = async (folderId: string, file: any, x: number) => {
      const fileId = v4();
    ...
    try {
      ...
      const foundItem = files?.find((i: any) => i.folderId);
      if (foundItem) {
        ...
        urls = await Promise.all(
          files.map((file: any, index) => {
            if (file?.type) {
              ...
              return addFileInCloud(folder, file, index);
            }
          })
        );
      } else {
      ...
      await axios.patch(
        backendUrl + "/api/v1/products/" + product._id,
        newProduct
      );
  ...
  return (
    <>
      <Container
        style={{
          transform: `scale(${!loadingProduct ? 1 : 0})`,
          opacity: !loadingProduct ? 1 : 0,
          borderRadius: !loadingProduct ? "0" : "500px",
      ...
      >
        ...
        >
          ...
          >
            <h1 style={{ fontSize: "24px" }}>{activeLanguage.editProduct}</h1>
          ...
          <FieldContainer>
            ...
            {variantsInput?.length > 0 && variants?.length > 0 && (
              ...
              >
                {variants
                  ?.filter((i: any) => i._id !== product?._id)
                  .map((item: any, index: number) => {
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      width: isMobile ? "95vw" : "312px",
                      height: isMobile ? "90vw" : "312px",
                      overflow: "hidden",
                      display: "flex",
                      alignItems: "center",
                      borderRadius: "20px",
                  ...
                  >
                    ...
                    <MdDelete
                      onClick={(e) => {
                        ...
                        if (item.type) {
                          setDeletePaths((prev) => [
                            ...prev,
                            `products/${item.folderId}/${item.itemId}`,
                          ]);
                        }
                        setFiles((prev) => {
                    ...
                    item?.type?.includes("video") ? (
                      <video
                        key={item?.url || URL.createObjectURL(item.blob)}
                        controls
                        autoPlay
                        loop
                        muted
                        style={{
                          ...
                          height: "100%",
                          objectFit: "cover",
                          borderRadius: "20px", // Match your img styles if necessary
                        }}
                      >
                        <source
                          src={item?.url || URL.createObjectURL(item?.blob)}
