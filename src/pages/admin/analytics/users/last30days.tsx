...
const DateRangePicker = () => {
  ...
  const { startDateVisit, setStartDateVisit, endDateVisit, setEndDateVisit } =
    useAdminContext();
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <DatePicker
            selected={startDateVisit}
            onChange={(date: any) => setStartDateVisit(date)}
            timeCaption="Time"
      ...
      >
        ...
        >
          <DatePicker
            selected={endDateVisit}
            onChange={(date: any) => setEndDateVisit(date)}
            timeCaption="Time"
            dateFormat="MMMM d, yyyy"
          />
        </DatePickerWrapper>
...
export default function Last30Days() {
  ...
  useEffect(() => {
    if (users?.daily) {
      const maxLength = users.daily.find((i: any) => i.maxLength)?.length || 1; // Avoid division by 0
      const calculatedPercentages = users.daily.map((item: any) => ({
        ...item,
