...
const DateRangePicker = () => {
  ...
  const { startDate, setStartDate, endDate, setEndDate } = useAdminContext();
...
export default function Last30Days() {
  ...
  useEffect(() => {
    if (stats?.orders?.daily) {
      const maxLength =
        stats.orders.daily.find((i: any) => i.maxOrders)?.totalOrders || 1; // Avoid division by 0
      const maxIncome =
        stats.orders.daily.find((i: any) => i.maxIncomes)?.totalIncomes || 1;

      const calculatedPercentages = stats.orders.daily.map((item: any) => ({
        ...item,
        ...
        >
          ...
          <div
            style={{
              ...
              padding: "2px",
              border: `1px solid ${
                activeType === "income" ? theme.primary : theme.lineDark
              }`,
              width: "30px",
              aspectRatio: 1,
              display: "flex",
