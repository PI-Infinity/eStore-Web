    ...
    >
      ...
      >
        ...
        >
          <DatePicker
            selected={startMonth}
            onChange={(date: any) => setStartMonth(date)}
            showMonthYearPicker
            dateFormat="MM/yyyy"
          />
        </DatePickerWrapper>
      ...
      >
        ...
        >
          <DatePicker
            selected={endMonth}
            onChange={(date: any) => setEndMonth(date)}
            showMonthYearPicker
            dateFormat="MM/yyyy"
          />
        </DatePickerWrapper>
...
export default function Monthly() {
  ...
  console.log(stats?.orders?.monthly);
  ...
  useEffect(() => {
    if (stats?.orders?.monthly) {
      const maxLength =
        stats.orders.monthly.find((i: any) => i.maxOrders)?.totalOrders || 1; // Avoid division by 0
      const maxIncome =
        stats.orders.monthly.find((i: any) => i.maxIncomes)?.totalIncomes || 1;

      const calculatedPercentages = stats.orders.monthly.map((item: any) => ({
        ...item,
        lengthHeightPercentage: (item.totalOrders / maxLength) * 80,
        incomeHeightPercentage: (item.totalIncomes / maxIncome) * 80,
      }));
      ...
      <Section style={{ border: `1px solid ${theme.lineDark}` }}>
        {loading ? (
          ...
          percentages?.map((item: any, index: number) => {
            const { lengthHeightPercentage, incomeHeightPercentage } = item;
            return (
              <div
                key={index}
                style={{
